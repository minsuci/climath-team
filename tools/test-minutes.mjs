...
// ---- 누가 볼 것인가 ----
// 프런트매터 «공개: 팀» 이라야 선생님도 본다. 안 적혀 있으면 팀장만 — **닫힌 쪽이 기본이다.**
// 간부회의가 실수로 열리는 것보다 팀회의가 실수로 닫히는 편이 낫다.
ok("공개를 안 적으면 팀장만", N.open === false, JSON.stringify(N.openWord));
const fm = (v) => parseNote("x.md", "---\n공개: " + v + "\n---\n\n# 제목\n").open;
ok("«공개: 팀» 이면 선생님도 본다", fm("팀") === true);
ok("«공개: 간부» 는 팀장만", fm("간부") === false);
ok("«공개: 팀장만» 도 팀장만", fm("팀장만") === false);
ok("빈 칸은 팀장만", fm("") === false);
// ⚠ 지문에 공개도 넣어야 한다 — 본문만 세면 공개 칸만 고쳤을 때 안 올라간다
{
  const body = "---\n공개: 팀\n---\n\n# 제목\n\n같은 본문\n";
  const shut = body.replace("공개: 팀", "공개: 간부");
  const h = (t) => { const n = parseNote("z.md", t); return n.md + "@공개=" + (n.open ? "팀" : "간부"); };
  ok("공개만 바뀌어도 지문이 달라진다", h(body) !== h(shut), h(body));
}
