...
const RENDER = { cal: "renderCal", tasks: "renderTasks", minutes: "renderMinutes", week: "renderWeek",
...
ok("메뉴가 아홉이다", pages.length === 9, String(pages.length));
pages.forEach((p) => {
  const f = RENDER[p[0]];
...
vm.runInContext(`S.claims = { tid: "T1", role: "owner" }; S.minutes = [];`, ctx);
