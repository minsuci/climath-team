...
//
// 마왕님 결정: 학생 단위 · 테스트 점수 한 칸(9/14 추가 — 처음엔 없음) · 마감 없음, 다음 날 아침 8시까지 안 내면 알림(팀장·본인) ·
...
run(`RP_WORK_DOWS_REAL = RP_WORK_DOWS;`);   // 아래 시험들이 상수를 비우므로 원본을 담아 둔다
run(`
...
// 출근 요일(RP_WORK_DOWS)은 아래 제 구역에서만 켠다 — 여기 시험들은 «아무도 안 적혀 있다(= 매일 출근)» 가 전제다
run(`RP_WORK_DOWS = {}`);
...
// ---- 출근 요일 (9/17 마왕님 «업무보고 출근 안 하는 날은 안 뜨도록». 한민수는 수·일 휴무) ----
// 한민수(T1)는 화·목 반 하나뿐이라 수·일은 원래 «매주 하는 일» 때문에 보고하는 날이었다. 안 나오는 날이면 그것도 아니다.
// ⚠ **출근하는 요일**을 적는다(마왕님이 그렇게 말해 준다). 수·일 휴무 = 월화목금토.
run(`RP_WORK_DOWS = { "한민수": [1, 2, 4, 5, 6] }`);
ok("안 적힌 사람은 모든 요일 출근으로 본다 (새 선생님이 조용히 빠지면 안 된다)", run(`return rpOffDow("T2", "2026-09-20")`) === false);
...
ok("받은 요일표 그대로 — 한민수 수일 휴무 · 정찬준 월~금 · 이창혁A 월~금+일 · 이현우 매일 · 박준성 월금 · 이승엽 화목",
  JSON.stringify(WORK) === JSON.stringify({ "한민수": [1,2,4,5,6], "정찬준": [1,2,3,4,5], "이창혁A": [0,1,2,3,4,5], "이현우": [0,1,2,3,4,5,6], "박준성": [1,5], "이승엽": [2,4] }), JSON.stringify(WORK));
...
ok("박준성 — 수(9/16)는 반이 있어도 중등관 날이라 안 센다", run(`return rpClassesOn("TJ", "2026-09-16").length`) === 1 && run(`return rpExpected("TJ", "2026-09-16")`) === false);
ok("박준성 — 금(9/18) 보고할 날 (금요일 업무보고에 뜬다)", run(`return rpExpected("TJ", "2026-09-18")`) === true);
...
(async () => {
  ...
  ok("점수를 아무도 안 적은 날에는 문서에 테스트 줄이 안 붙는다", !/테스트|점수/.test(dg));
